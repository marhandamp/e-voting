...
import com.app.myvoting.databinding.ActivityScanNfcBinding
import com.app.myvoting.parser.NdefMassageParser
        ...
        RetrofitClient.instance.getPemilih().enqueue(object : Callback<ModelResultPemilihData> {
            override fun onResponse(
                ...
                for (i in 0 until size){
                    ...
                    if (data == data3){
                        id = response.body()!!.modelResultPemilih[i].id
                        idNfc = response.body()!!.modelResultPemilih[i].idNfc
                        noTps = response.body()!!.modelResultPemilih[i].noTps
                    }
                }

                if (data3 == idNfc){
                    val intent = Intent(this@ScanNfcActivity, KandidatActivity::class.java)
                    intent.putExtra("ID", id)
                    intent.putExtra("NO_TPS", noTps)
                    startActivity(intent)
                } else {
                    Toast.makeText(this@ScanNfcActivity, "KTP Tidak Terdaftar", Toast.LENGTH_SHORT).show()
                }
            }
